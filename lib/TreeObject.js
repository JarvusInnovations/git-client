...
class TreeObject {
    ...
    async write (basePath = '.') {
        if (!this.dirty) {
            return this.hash;
        }

        console.log(`writing dirty tree ${basePath}`);
        ...
        for (const name in this.children) {
            ...
            if (child.isTree) {
                lines.push(`040000 tree ${child.dirty ? await child.write(require('path').join(basePath, name)) : child.hash}\t${name}`);
            } else {
    ...
    async mergeOverlay (input, basePath = '.') {
        ...
        for (const childName in inputChildren) {
            ...
            console.log(`merging ${require('path').join(basePath, childName)} ${baseChild.hash} <- ${inputChild.hash}`);
            const mergePromise = baseChild.mergeOverlay(inputChild, require('path').join(basePath, childName));
