...
class Git {
    ...
    createBlob (properties) {
        return new BlobObject(properties, this);
    }

    createTree (properties) {
        return new TreeObject(properties, this);
    }

    async createTreeFromRef (ref) {
        try {
            const refHash = await this.revParse({ verify: true }, ref);
            const treeHash = await this.revParse({ verify: true }, `${refHash}^{tree}`);
            return new TreeObject({ hash: treeHash }, this);
        } catch (err) {
     ...
     */
    async getIndexPath () {
        return await this.revParse({ 'git-path': true }, 'index');
    }
    ...
    async mktreeBatch(children) {
        ...
        if (!this._mktreeBatchProcess) {
            ...
            this._mktreeBatchProcess.on('exit', code => {
                ...
                if (code == 0) {
                    ...
                    const err = new Error('mktree failed');
                    err.output = output;
                    err.code = code;
        ...
        return new Promise((resolve, reject) => {
            ...
            this._mktreeBatchTimeout = setTimeout(() => this._mktreeBatchProcess.stdin.end(), 5000);

            this._mktreeBatchQueue.push({ resolve, reject, output: [] });
        });
    }
