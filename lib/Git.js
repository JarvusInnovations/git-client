...
class Git {
    ...
    async writeLooseObject (hash, content) {
        ...
        const dirPath = nodepath.join(gitDir, 'objects', prefix);
        const objPath = nodepath.join(dirPath, suffix);

        // skip if already on disk
        if (await fs.exists(objPath)) {
            return hash;
        }
    ...
    /**
     * Computes a tree hash and writes the tree object natively (no subprocess)
     *
     * Git tree format: sorted entries of `{mode} {name}\0{20-byte-hash}`
     * Object format: `tree {content-length}\0{content}`
     *
    ...
    async mktreeNative (children) {
        ...
        const sorted = children.slice().sort((a, b) => {
            ...
            for (let i = 0; i < len; i++) {
                const ca = nameA.charCodeAt(i);
                const cb = nameB.charCodeAt(i);
                if (ca !== cb) return ca - cb;
            }

            const ca = nameA.length > len ? nameA.charCodeAt(len) : (a.type === 'tree' ? 0x2F : 0);
            const cb = nameB.length > len ? nameB.charCodeAt(len) : (b.type === 'tree' ? 0x2F : 0);
            return ca - cb;
        });

        // build binary tree content
        // git stores modes without leading zeros (e.g. '040000' -> '40000')
        const parts = [];
        for (const { mode, hash, name } of sorted) {
            const normalizedMode = mode.replace(/^0+/, '') || '0';
            parts.push(Buffer.from(`${normalizedMode} ${name}\0`));
        ...
        const full = Buffer.concat([header, treeContent]);
    ...
    async mktreeBatch(children) {
        ...
        if (!this._mktreeBatchProcess) {
            this._mktreeBatchProcess = await this.mktree({ batch: true, missing: true, $spawn: true });
            ...
            this._mktreeBatchProcess.stdout.on('data', data => {
                ...
                if (data.includes('\n')) {
                    ...
                    logger.debug('git mktree --batch --missing -> %s', output);
